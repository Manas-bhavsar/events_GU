...
export default function EventSlider({ images, intervalMs = 4000 }: EventSliderProps) {
  ...
  return (
    <div className="w-full">
      <div
        className="relative w-full aspect-[16/9] rounded-lg overflow-hidden bg-white/10"
        onMouseEnter={stop}
        onMouseLeave={start}
      >
        {current?.src ? (
        ...
        <div className="mt-3 flex items-center gap-2 overflow-x-auto">
          {validImages.map((img, i) => (
            <button
              ...
              type="button"
              onClick={() => goto(i)}
              className={`relative shrink-0 w-16 h-12 rounded-md overflow-hidden border ${
                i === index ? 'border-accent-orange ring-2 ring-accent-orange/40' : 'border-white/20'
              }`}
